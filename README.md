...
- If downloads are disabled, each person with external OBJE URLs gets a `## External media` section in their person note listing titles and the original URLs. This makes it safe and easy to review external sources without triggering any downloads.
- When downloads are enabled, the tool attempts polite, concurrent downloads with retries, backoff, and per-host limits. If a download fails or exceeds size limits, the original URL is retained in the person note.
